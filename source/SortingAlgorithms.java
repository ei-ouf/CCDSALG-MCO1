...
public class SortingAlgorithms {
    ...
    private long comparisonCount = 0; //count number of comparisons
    long swapCount = 0; //count number of swaps
    ...
    public void insertionSort(Record[] arr, int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            while (j >= 0 && arr[j].getIdNumber() > key.getIdNumber()) {
                comparisonCount++; //count number of comparisons
                arr[j + 1] = arr[j]; //move it to the right, it's okay to override the ar[i]
                swapCount++; //count number of swaps
    ...
    private void merge(Record[] arr, int p, int q, int r) {
        ...
        System.arraycopy(arr, p, left, 0, lSize);
